...
const App: React.FC = () => {
  ...
  const runAnalysis = useCallback(async () => {
    ...
    const progressInterval = setInterval(() => {
      setLoadingProgress(prev => {
        ...
        const step = Math.max(0.1, Math.random() * (100 - prev) / 10);
    ...
    try {
      ...
      clearInterval(progressInterval);
      setError(err.message || "Strategic synthesis interrupted.");
      setIsAnalyzing(false);
    }
  }, [activeDocuments, meetingContext, analysis, generateStateHash]);

  const loadingStatusText = useMemo(() => {
    if (loadingProgress < 20) return "Parsing Intellectual Nodes...";
    if (loadingProgress < 40) return "Context Mapping Active...";
    if (loadingProgress < 60) return "Synthesizing Buyer Psychology...";
    if (loadingProgress < 80) return "Modeling Decision Wedges...";
    return "Finalizing Strategic Brief...";
  }, [loadingProgress]);

  const reset = () => {
    if(confirm("Wipe current strategy context?")) {
      setFiles([]);
      setSelectedLibraryDocIds([]);
      setAnalysis(null);
  ...
  if (authLoading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50">
        ...
        <p className="mt-6 text-[10px] font-black uppercase text-slate-400 tracking-widest animate-pulse">Establishing Neural Link...</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <style>{`
        :root { --text-zoom-multiplier: ${textZoom / 100}; }
        .text-magnifier p, .text-magnifier span:not(.no-zoom), .text-magnifier h1, .text-magnifier h2, 
        .text-magnifier h3, .text-magnifier h4, .text-magnifier h5, .text-magnifier h6, .text-magnifier li, 
        .text-magnifier button:not(.no-zoom), .text-magnifier input, .text-magnifier textarea {
      ...
      <Header 
        ...
        companyName={meetingContext.sellerCompany}
      />
        ...
        <div className="flex flex-1 overflow-hidden bg-white relative">
          {analysis && !isAnalyzing && (
            <>
              ...
              >
                <div className={`p-2 lg:p-6 space-y-8 flex flex-col h-full`}>
                  <div className="space-y-1">
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <SidebarBtn active={activeTab === 'audio'} onClick={() => setActiveTab('audio')} icon={<ICONS.Speaker />} label="Studio" scale={sidebarFontScale} />
                      <SidebarBtn active={activeTab === 'gpt'} onClick={() => setActiveTab('gpt')} icon={<ICONS.Sparkles />} label="Fast Ans" scale={sidebarFontScale} />
                      <SidebarBtn active={activeTab === 'context'} onClick={() => setActiveTab('context')} icon={<ICONS.Efficiency />} label="Config" scale={sidebarFontScale} />
                    </div>
                  </div>
            ...
            <div className="w-full min-h-full">
              {!analysis && !isAnalyzing ? (
                <div className="p-8 md:p-12 space-y-16 animate-in fade-in slide-in-from-top-4 duration-500 max-w-7xl mx-auto">
                  <div className="text-center flex flex-col items-center gap-12 py-10">
                    <div className="p-10 bg-indigo-600 text-white rounded-[3rem] shadow-[0_40px_80px_-15px_rgba(79,70,229,0.3)] animate-in zoom-in duration-700">
                       <ICONS.Trophy className="w-32 h-32" />
                    </div>
                    <h1 className="text-[10rem] leading-[0.8] font-black text-slate-900 tracking-tighter uppercase break-words max-w-full">
                      {meetingContext.sellerCompany || "IDENTITY CORE"}
                    </h1>
                  </div>
                    ...
                    <div className="mt-12 flex flex-col items-center gap-6">
                      {error && (
                        <div className="bg-rose-50 border border-rose-100 rounded-2xl p-6 mb-8 max-w-xl text-center">
                          <p className="text-rose-600 font-bold mb-2">⚠️ Process Interrupted</p>
                      ...
                      >
                        <ICONS.Brain />
                        {isAnyFileProcessing ? 'Ingesting Intel...' : 'Generate Full Strategy'}
                      </button>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col items-center justify-center space-y-12 h-full min-h-[600px]">
                  <div className="relative">
                    <div className="absolute inset-0 bg-indigo-500/20 blur-[60px] rounded-full" style={{ transform: `scale(${1 + (loadingProgress / 100)})`, opacity: 0.2 + (loadingProgress / 100) }}></div>
                    <div className="relative w-32 h-32 border-4 border-indigo-50 border-t-indigo-600 rounded-full animate-spin"></div>
                    <div className="absolute inset-0 flex items-center justify-center text-indigo-600"><ICONS.Brain className="w-10 h-10" /></div>
                    <div className="absolute -bottom-10 left-1/2 -translate-x-1/2">
                      <span className="text-3xl font-black text-slate-800 tracking-tighter">{Math.floor(loadingProgress)}%</span>
                    </div>
                  </div>
                  ...
                  {activeTab === 'context' && (
                    <div className="p-8 md:p-12 space-y-12 max-w-7xl mx-auto">
                      ...
                      <div className="flex justify-center pb-10">
                        <button onClick={runAnalysis} disabled={isAnalyzing} className="px-12 py-5 bg-indigo-600 text-white rounded-full font-black text-lg shadow-2xl hover:bg-indigo-700 transition-all hover:scale-105 active:scale-95 flex items-center gap-3"><ICONS.Brain />Update Intelligence</button>
                      </div>
                    </div>
                  )}
...
const SidebarBtn = ({ active, onClick, icon, label, scale = 1 }: { active: boolean; onClick: () => void; icon: React.ReactNode; label: string, scale?: number }) => (
  ...
  >
    ...
    <span className="tracking-tight truncate uppercase text-[10px] font-black">{label}</span>
  </button>
);
