...
const ALL_ANSWER_STYLES = [
  ...
  "Key Statistics", "Case Study Summary", "Competitive Comparison"
];
...
const App: React.FC = () => {
  ...
  const runAnalysis = useCallback(async () => {
    if (activeDocuments.length === 0) {
      setError("Document ingestion required for synthesis.");
      return;
    }
    ...
    const progressInterval = setInterval(() => {
      setLoadingProgress(prev => (prev < 95 ? prev + Math.random() * 2 : prev));
    }, 200);
    ...
    try {
      ...
      clearInterval(progressInterval);
      setError(err.message || "Cognitive synthesis failed.");
      setIsAnalyzing(false);
    }
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFD] flex flex-col text-slate-900">
      ...
      <div className="flex flex-1 pt-16 h-screen">
        ...
        <aside className="w-80 bg-white border-r border-slate-100 flex flex-col p-6 space-y-8 z-40 fixed h-[calc(100vh-64px)]">
          <div className="space-y-1">
            ...
            <nav className="flex flex-col gap-1.5">
              ...
              <SidebarItem active={activeTab === 'avatar2'} onClick={() => setActiveTab('avatar2')} icon={<ICONS.Brain />} label="Avatar Sim 2.0" />
          ...
          <div className="mt-auto space-y-4">
            <div className="bg-red-50 p-6 rounded-[2rem] border border-red-100 shadow-sm shadow-red-100/50">
               ...
               <p className="text-[11px] font-bold text-red-900 leading-relaxed">
                 {analysis ? `Synced with ${meetingContext.clientCompany || 'Prospect'}` : 'Awaiting Documentary Ingestion'}
               </p>
            </div>
            <button 
        ...
        {/* Main Content Area */}
        <main className="flex-1 ml-80 overflow-y-auto no-scrollbar bg-[#FDFDFD]">
          <div className="max-w-6xl mx-auto p-12">
            {isAnalyzing ? (
              <div className="h-[70vh] flex flex-col items-center justify-center space-y-10 animate-in fade-in zoom-in-95 duration-500">
                ...
                <div className="text-center space-y-4">
                  ...
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] animate-pulse">Cognitive Node Alignment: {Math.round(loadingProgress)}%</p>
                </div>
              </div>
            ) : (
                  ...
                  <div className="space-y-12">
                    ...
                    <div className="flex justify-center pt-8">
                       <button 
                        ...
                        className="px-16 py-6 bg-red-600 text-white rounded-full font-black text-xl shadow-2xl shadow-red-200 hover:bg-red-700 hover:scale-105 active:scale-95 transition-all flex items-center gap-4"
                       >
...
const SidebarItem = ({ active, onClick, icon, label }: { active: boolean; onClick: () => void; icon: React.ReactNode; label: string }) => (
  <button 
    onClick={onClick}
    className={`
      w-full flex items-center gap-4 px-6 py-4 rounded-2xl font-bold text-sm transition-all group
      ${active ? 'bg-red-600 text-white shadow-xl shadow-red-200' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'}
    `}
  >
