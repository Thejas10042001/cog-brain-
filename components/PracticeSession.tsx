...
export const PracticeSession: React.FC<PracticeSessionProps> = ({ analysis, meetingContext, onStartSimulation }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border-y border-slate-200 dark:border-slate-800 p-6 md:p-12 shadow-2xl overflow-hidden relative min-h-[calc(100vh-64px)] flex flex-col transition-colors duration-500">
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-12">
        <div className="flex items-center gap-4">
          <div className="p-3.5 bg-rose-600 text-white rounded-2xl shadow-xl shadow-rose-100 dark:shadow-none"><ICONS.Speaker /></div>
          <div>
            <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Performance Grooming Lab</h3>
            <p className="text-[10px] text-slate-400 dark:text-slate-500 font-black uppercase tracking-widest">Master Your Verbal Architecture</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          <button 
            ...
            className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all border ${showGroomingJournal ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 border-indigo-100 dark:border-indigo-900/30'}`}
          >
            {showGroomingJournal ? 'Close Journal' : 'Self-Grooming Journal'}
          </button>
          <div className="flex gap-2 p-1.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl">
            <button 
              onClick={() => { stopPractice(); setSessionMode('roleplay'); setEvaluation(null); }}
              className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${sessionMode === 'roleplay' ? 'bg-indigo-600 text-white shadow-xl' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300'} ${highlightedButton === 'roleplay' ? 'ring-4 ring-indigo-400 animate-pulse' : ''}`}
            >
              Buyer Roleplay
            </button>
            <button 
              onClick={() => { stopPractice(); setSessionMode('seller-roleplay'); setEvaluation(null); }}
              className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${sessionMode === 'seller-roleplay' ? 'bg-indigo-600 text-white shadow-xl' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300'} ${highlightedButton === 'seller-roleplay' ? 'ring-4 ring-indigo-400 animate-pulse' : ''}`}
            >
              Seller Roleplay
            </button>
            <button 
              onClick={() => { stopPractice(); setSessionMode('grooming'); }}
              className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${sessionMode === 'grooming' ? 'bg-indigo-600 text-white shadow-xl' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300'} ${highlightedButton === 'grooming' ? 'ring-4 ring-indigo-400 animate-pulse' : ''}`}
            >
              Bot-Led Grooming
            </button>
          </div>
      ...
      {showGroomingJournal ? (
        <div className="flex-1 space-y-8 animate-in fade-in zoom-in-95 duration-500 overflow-y-auto custom-scrollbar pb-12">
           <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-6">
              <h4 className="text-xl font-black text-slate-900 dark:text-white tracking-tight">Your Self-Grooming Journal</h4>
              <span className="text-[9px] font-black uppercase text-indigo-500 dark:text-indigo-400 tracking-widest bg-indigo-50 dark:bg-indigo-900/30 px-3 py-1 rounded-lg">{savedGroomings.length} Saved Protocols</span>
           </div>
           {savedGroomings.length === 0 ? (
             <div className="flex flex-col items-center justify-center py-24 opacity-30 text-center space-y-4">
                <ICONS.Document className="w-16 h-16 dark:text-white" />
                <p className="text-sm font-bold uppercase tracking-widest dark:text-white">Journal Empty. Add your first audit for self-grooming.</p>
             </div>
           ) : (
             <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {savedGroomings.map(saved => (
                  <div key={saved.id} className="p-8 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 rounded-[2.5rem] relative group hover:bg-white dark:hover:bg-slate-800 hover:border-indigo-200 dark:hover:border-indigo-700 transition-all hover:shadow-2xl">
                     <div className="flex items-center justify-between mb-4">
                        <span className="text-[8px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest bg-indigo-50 dark:bg-indigo-900/30 px-3 py-1 rounded-full">{new Date(saved.timestamp).toLocaleDateString()}</span>
                        <button 
                          onClick={() => setSavedGroomings(prev => prev.filter(p => p.id !== saved.id))}
                          className="text-slate-300 dark:text-slate-600 hover:text-rose-50 dark:hover:text-rose-400"
                        ><ICONS.X className="w-4 h-4" /></button>
                     </div>
                     <p className="text-sm font-black text-slate-900 dark:text-white mb-2 truncate">Q: {saved.question}</p>
                     <p className="text-[11px] font-bold text-slate-500 dark:text-slate-400 italic mb-6 line-clamp-2">"{saved.evaluation.idealWording}"</p>
                     <button 
                       onClick={() => { setEvaluation(saved.evaluation); setShowGroomingJournal(false); }}
                       className="text-[10px] font-black uppercase text-indigo-600 dark:text-indigo-400 tracking-widest flex items-center gap-2 hover:translate-x-1 transition-transform"
                     >
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center space-y-12 w-full mx-auto py-12">
          <div className="space-y-4">
            <div className={`w-24 h-24 rounded-[2.5rem] flex items-center justify-center mx-auto mb-8 shadow-2xl transition-transform hover:scale-105 duration-500 ${sessionMode === 'roleplay' ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400' : 'bg-rose-50 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400'}`}>
            ...
            <p className="text-slate-500 dark:text-slate-400 text-lg leading-relaxed max-w-2xl mx-auto font-medium">
          ...
          <div className="w-full space-y-10">
             {sessionMode === 'grooming' ? (
               <div className="space-y-4 max-w-2xl mx-auto text-left">
                  <label className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 dark:text-slate-500 ml-2">Target Objection / Question</label>
                  <select 
                    value={groomingTarget}
                    onChange={(e) => setGroomingTarget(e.target.value)}
                    className="w-full bg-slate-50 dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 rounded-[1.5rem] px-8 py-5 text-base font-bold text-slate-800 dark:text-white outline-none focus:border-indigo-500 focus:bg-white dark:focus:bg-slate-900 transition-all shadow-inner"
                  >
                    <optgroup label="Critical Objections" className="dark:bg-slate-900">
                      {analysis.objectionHandling.map((o, i) => <option key={i} value={o.objection}>{o.objection}</option>)}
                    </optgroup>
                    <optgroup label="Anticipated Questions" className="dark:bg-slate-900">
               ...
               <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 px-12">
                 {PERSONA_OPTIONS.map((option) => (
                   <button
                     ...
                     className={`p-8 rounded-[2.5rem] border-2 text-left transition-all relative overflow-hidden group flex flex-col h-full ${selectedPersona === option.type ? 'bg-indigo-600 border-indigo-600 text-white shadow-xl scale-[1.03]' : 'bg-white dark:bg-slate-900/50 border-slate-100 dark:border-slate-800 hover:border-indigo-300 dark:hover:border-indigo-700 shadow-sm'}`}
                   >
                     <div className={`p-4 rounded-2xl mb-6 inline-block w-fit ${selectedPersona === option.type ? 'bg-white/20 text-white' : 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 shadow-sm'}`}>{option.icon}</div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center space-y-8 animate-in fade-in duration-700">
           <div className="relative">
              <div className="w-24 h-24 border-8 border-indigo-50 dark:border-indigo-900/30 border-t-indigo-600 rounded-full animate-spin"></div>
              <div className="absolute inset-0 flex items-center justify-center text-indigo-600 scale-125"><ICONS.Brain /></div>
           </div>
           <div className="text-center">
              <p className="text-3xl font-black text-slate-900 dark:text-white tracking-tight mb-2">Cognitive Mastery Audit In Progress</p>
              <p className="text-[11px] text-slate-400 dark:text-slate-500 font-black uppercase tracking-[0.4em] animate-pulse">Analyzing Grammar • Tone • Structure • Pacing</p>
           </div>
        </div>
      ) : evaluation ? (
        <div className="flex-1 space-y-12 animate-in slide-in-from-bottom-8 duration-1000 pb-20 w-full px-12">
          <div className="flex items-center justify-between">
             <button onClick={() => setEvaluation(null)} className="text-[11px] font-black uppercase text-indigo-600 dark:text-indigo-400 tracking-widest flex items-center gap-2 hover:translate-x-[-4px] transition-transform">
               <ICONS.X /> Close Mastery Review
             </button>
             <div className="flex items-center gap-4">
                <button 
                  onClick={addToGroomingJournal}
                  className="px-6 py-2.5 bg-emerald-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-700 shadow-xl shadow-emerald-100 dark:shadow-none flex items-center gap-2"
                >
                  <ICONS.Efficiency className="w-4 h-4" /> Add to Journal
                </button>
                <div className="px-6 py-2.5 bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-100 dark:border-indigo-800 text-indigo-600 dark:text-indigo-400 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            <div className="space-y-8">
               <div className="p-10 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 rounded-[3rem] shadow-inner relative overflow-hidden group">
                  <h4 className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 tracking-widest mb-6">Recorded Performance</h4>
                  <p className="text-lg font-medium leading-relaxed italic text-slate-700 dark:text-slate-300">“{evaluation.transcription}”</p>
               </div>

                <div className="p-10 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-[3rem] shadow-2xl relative overflow-hidden group">
                  <h4 className="text-[10px] font-black uppercase text-indigo-600 dark:text-indigo-400 tracking-widest mb-6 flex items-center gap-2">
                    <ICONS.Sparkles className="w-4 h-4" /> Tactical Breathing & Pacing Guide
                  </h4>
                  <p className="text-xl font-medium leading-[2.2] text-slate-700 dark:text-slate-300 font-serif italic">
                    {evaluation.breathPacingGuide.split(/(\[Take Breath\]|\[Pause - \d+s\]|\[Slow Down\])/g).map((part, i) => (
                      ...
                      ? <span key={i} className="bg-indigo-600/10 dark:bg-indigo-600/20 text-indigo-600 dark:text-indigo-400 px-3 py-1 rounded-xl mx-1 font-black text-[10px] uppercase tracking-widest not-italic border border-indigo-200 dark:border-indigo-800 shadow-sm">{part}</span>
                      : part
                    ))}
                  </p>
               </div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <div className="p-8 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 rounded-[2.5rem] space-y-4">
                    <h5 className="text-[9px] font-black uppercase text-indigo-600 dark:text-indigo-400 tracking-widest">Sentence Formation Audit</h5>
                    <p className="text-[11px] font-bold text-slate-700 dark:text-slate-300 leading-relaxed italic">{evaluation.sentenceFormation}</p>
                 </div>
                 <div className="p-8 bg-rose-50 dark:bg-rose-900/20 border border-rose-100 dark:border-rose-800 rounded-[2.5rem] space-y-4">
                    <h5 className="text-[9px] font-black uppercase text-rose-600 dark:text-rose-400 tracking-widest">Vocal Tone & Pace Audit</h5>
                    <p className="text-[11px] font-bold text-slate-700 dark:text-slate-300 leading-relaxed italic">{evaluation.toneAnalysis}</p>
                 </div>
               </div>
            </div>

            <div className="space-y-8">
               <div className="p-12 bg-white dark:bg-slate-900 border-4 border-indigo-50 dark:border-indigo-900/30 rounded-[4rem] shadow-2xl relative overflow-hidden group/master">
                  <div className="relative z-10">
                    <div className="flex items-center justify-between mb-12">
                       <h4 className="text-[13px] font-black uppercase text-indigo-600 dark:text-indigo-400 tracking-[0.4em]">Optimized Ideal wording</h4>
                       <span className="text-[9px] font-black text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 px-4 py-1.5 rounded-full uppercase border border-emerald-100 dark:border-emerald-800">Validated Logic</span>
                    </div>
                    
                    <p className="text-3xl font-black text-slate-900 dark:text-white leading-tight mb-12 tracking-tight">“{evaluation.idealWording}”</p>
                    
                    <div className="p-8 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 rounded-[2.5rem] mb-12">
                       <h5 className="text-[10px] font-black uppercase text-slate-500 dark:text-slate-400 mb-4">Linguistic Corrections</h5>
                       <p className="text-sm font-bold text-slate-700 dark:text-slate-300 leading-relaxed whitespace-pre-wrap">{evaluation.grammarFeedback}</p>
                    </div>
                    ...
                    <div className="flex flex-col gap-4">
                       <button 
                         ...
                         className={`w-full flex items-center justify-center gap-5 py-7 rounded-[2rem] font-black text-base uppercase tracking-widest shadow-2xl transition-all active:scale-95 ${isPlayingIdeal ? 'bg-indigo-400 text-white' : 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-indigo-100 dark:shadow-none'}`}
                       >
                       ...
                       <button 
                         ...
                         disabled={isPlayingIdeal || isPlayingExplanation}
                         className={`w-full flex items-center justify-center gap-5 py-7 rounded-[2rem] font-black text-[11px] uppercase tracking-widest border-2 transition-all active:scale-95 ${isPlayingExplanation ? 'text-slate-400 dark:text-slate-600' : 'text-slate-700 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 border-slate-200 dark:border-slate-700 hover:border-indigo-200 dark:hover:border-indigo-800 shadow-lg'}`}
                       >
               ...
               <div className="p-10 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 rounded-[3.5rem] animate-in fade-in zoom-in-95 duration-1000 shadow-sm">
                  <div className="flex items-center gap-3 mb-6">
                     <div className="p-3 bg-emerald-600 text-white rounded-xl shadow-lg shadow-emerald-100 dark:shadow-none"><ICONS.Brain /></div>
                     <h4 className="text-[11px] font-black uppercase text-emerald-600 dark:text-emerald-400 tracking-[0.3em]">Self-Grooming Explanation</h4>
                  </div>
                  <p className="text-base font-medium text-emerald-950 dark:text-emerald-50 leading-relaxed whitespace-pre-wrap italic">
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-0 overflow-hidden h-full">
          <div className="lg:col-span-2 bg-white dark:bg-slate-900 p-12 flex flex-col items-center justify-center relative shadow-2xl overflow-hidden">
            ...
            <div className="relative w-80 h-80 mb-12 flex items-center justify-center">
               <div className={`absolute inset-0 bg-indigo-50 dark:bg-indigo-900/20 rounded-full ${isActive ? 'animate-ping' : 'animate-pulse'} scale-[1.4]`}></div>
               <div className={`w-40 h-40 bg-indigo-600 rounded-full flex items-center justify-center text-white scale-[1.7] shadow-[0_0_80px_rgba(79,70,229,0.5)] z-10 border-8 border-white dark:border-slate-800 transition-transform ${isActive ? 'animate-pulse' : ''}`}>
            ...
            <div className="text-center space-y-6 relative z-10 max-w-xl">
                <span className="px-5 py-2 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 text-[10px] font-black uppercase tracking-[0.3em] rounded-xl border border-indigo-100 dark:border-indigo-800 mb-4 inline-block">
                ...
                <h5 className="text-slate-900 dark:text-white text-4xl font-black tracking-tight leading-tight">
                  {sessionMode === 'roleplay' ? buyerName : sessionMode === 'seller-roleplay' ? sellerName : 'Neural Bot-Coach'}
                </h5>
                <p className="text-slate-500 dark:text-slate-400 text-lg italic font-medium leading-relaxed">
                  {sessionMode === 'roleplay' 
          ...
          <div className="bg-slate-50 dark:bg-slate-900/50 p-10 flex flex-col border-l border-slate-200 dark:border-slate-800 overflow-hidden shadow-inner relative h-full">
            <div className="flex items-center justify-between mb-8">
              <h6 className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400 dark:text-slate-500 flex items-center gap-3">
            ...
            <div className="flex-1 overflow-y-auto space-y-8 custom-scrollbar pr-6">
              {transcription.length === 0 && !currentTranscription.user && !currentTranscription.ai && (
                <div className="py-24 text-center space-y-6 opacity-20">
                   <ICONS.Speaker className="mx-auto w-14 h-14 dark:text-white" />
                   <p className="text-[11px] font-black uppercase tracking-[0.4em] dark:text-white">Voice Interaction Disabled</p>
                </div>
              )}
              {transcription.map((turn, i) => (
                <div key={i} className="space-y-3 animate-in slide-in-from-bottom-4 duration-500">
                  <div className="flex flex-col items-end text-right">
                    <p className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1.5">Your input</p>
                    <p className="text-sm text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 p-6 rounded-[2rem] rounded-tr-none border border-slate-200 dark:border-slate-700 shadow-md leading-relaxed font-medium w-full">“{turn.user}”</p>
                  </div>
                  {turn.ai && (
                    <div className="flex flex-col items-start text-left">
                      <p className="text-[9px] font-black text-indigo-500 dark:text-indigo-400 uppercase tracking-widest mb-1.5">AI Response</p>
                      <p className="text-sm text-indigo-950 dark:text-indigo-50 bg-indigo-50 dark:bg-indigo-900/30 p-6 rounded-[2rem] rounded-tl-none border border-indigo-200 dark:border-indigo-800 font-bold shadow-md leading-relaxed w-full">“{turn.ai}”</p>
                    </div>
                  )}
                </div>
              ))}
              {(currentTranscription.user || currentTranscription.ai) && (
                <div className="space-y-3 animate-pulse">
                  {currentTranscription.user && (
                    <div className="flex flex-col items-end text-right">
                      <p className="text-[9px] font-black text-slate-300 dark:text-slate-600 uppercase tracking-widest mb-1.5">Hearing...</p>
                      <p className="text-sm text-slate-400 dark:text-slate-500 bg-white/50 dark:bg-slate-800/50 p-6 rounded-[2rem] rounded-tr-none border border-dashed border-slate-200 dark:border-slate-700 leading-relaxed italic w-full">“{currentTranscription.user}”</p>
                    </div>
                  )}
                  {currentTranscription.ai && (
                    <div className="flex flex-col items-start text-left">
                      <p className="text-[9px] font-black text-indigo-300 dark:text-indigo-600 uppercase tracking-widest mb-1.5">Responding...</p>
                      <p className="text-sm text-indigo-300 dark:text-indigo-400 bg-indigo-50/30 dark:bg-indigo-900/10 p-6 rounded-[2rem] rounded-tl-none border border-dashed border-indigo-100 dark:border-indigo-900 leading-relaxed italic w-full">“{currentTranscription.ai}”</p>
                    </div>
                  )}
                </div>
              )}
