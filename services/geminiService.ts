...
// Cognitive Pro: Multi-style strategic synthesis
export async function* streamCognitivePro(
  prompt: string, 
  history: GPTMessage[], 
  ...
  const systemInstruction = `You are Cognitive Pro, the most advanced strategic synthesis engine.
  
  USER REQUESTED STYLES: ${selectedOptions.join(', ')}
  
  MISSION: Synthesize a world-class response that incorporates ALL the requested styles seamlessly.
  
  STYLE GUIDELINES:
  ${selectedOptions.includes('Psychological Projection') ? '- Psychological Projection: Analyze the underlying psychological drivers of the stakeholders.' : ''}
  ${selectedOptions.includes('Executive Summary') ? '- Executive Summary: Provide a high-level overview for leadership.' : ''}
  ${selectedOptions.includes('Analogy Based') ? '- Analogy Based: Use powerful analogies to simplify complex concepts.' : ''}
  ${selectedOptions.includes('Data-Driven Insights') ? '- Data-Driven Insights: Prioritize hard data and metrics.' : ''}
  ${selectedOptions.includes('Concise Answer') ? '- Concise Answer: Be extremely brief and direct.' : ''}
  ${selectedOptions.includes('In-Depth Response') ? '- In-Depth Response: Provide exhaustive detail.' : ''}
  ${selectedOptions.includes('Answer in Points') ? '- Answer in Points: Use bulleted lists for clarity.' : ''}
  ${selectedOptions.includes('Define Technical Terms') ? '- Define Technical Terms: Ensure all jargon is explained.' : ''}
  ${selectedOptions.includes('Sales Points') ? '- Sales Points: Highlight key selling features.' : ''}
  ${selectedOptions.includes('Key Statistics') ? '- Key Statistics: Include relevant industry or deal stats.' : ''}
  ${selectedOptions.includes('Case Study Summary') ? '- Case Study Summary: Reference relevant success stories.' : ''}
  ${selectedOptions.includes('Competitive Comparison') ? '- Competitive Comparison: Contrast against major competitors.' : ''}
  ${selectedOptions.includes('Anticipated Customer Questions') ? '- Anticipated Customer Questions: Predict and answer likely follow-ups.' : ''}
  ${selectedOptions.includes('Information Gap') ? '- Information Gap: Identify what we DON\'T know yet.' : ''}
  ${selectedOptions.includes('Pricing Overview') ? '- Pricing Overview: Discuss financial structures.' : ''}
  ${selectedOptions.includes('ROI Forecast') ? '- ROI Forecast: Project financial returns.' : ''}
  ${selectedOptions.includes('SWOT Analysis') ? '- SWOT Analysis: Provide Strengths, Weaknesses, Opportunities, Threats.' : ''}
  ${selectedOptions.includes('Strategic Roadmap') ? '- Strategic Roadmap: Outline the path forward.' : ''}
  ${selectedOptions.includes('Risk Assessment') ? '- Risk Assessment: Identify and mitigate deal risks.' : ''}
  ${selectedOptions.includes('Implementation Timeline') ? '- Implementation Timeline: Detail the rollout phases.' : ''}
  ${selectedOptions.includes('Technical Deep-Dive') ? '- Technical Deep-Dive: Provide low-level technical architecture details.' : ''}
  ${selectedOptions.includes('Value Proposition') ? '- Value Proposition: Clearly state the unique value.' : ''}
  ${selectedOptions.includes('Financial Justification') ? '- Financial Justification: Build the business case for the spend.' : ''}
  ${selectedOptions.includes('Stakeholder Alignment') ? '- Stakeholder Alignment: Address the needs of different departments.' : ''}
  ${selectedOptions.includes('Competitive Wedge') ? '- Competitive Wedge: Identify the specific point of differentiation.' : ''}
  ${selectedOptions.includes('Success Story Summary') ? '- Success Story Summary: Summarize a relevant win.' : ''}
  ${selectedOptions.includes('Buying Fear Mitigation') ? '- Buying Fear Mitigation: Address the psychological barriers to purchase.' : ''}
  ${selectedOptions.includes('Security & Compliance') ? '- Security & Compliance: Address data protection and regulatory needs.' : ''}
  ${selectedOptions.includes('Decision Matrix') ? '- Decision Matrix: Provide a framework for the client to choose.' : ''}
  ${selectedOptions.includes('Reasoning Chain') ? '- Reasoning Chain: Show the step-by-step logic used to reach the conclusion.' : ''}
...
export async function suggestCognitiveProOptions(prompt: string, context?: string): Promise<string[]> {
  const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
  const modelName = 'gemini-3-flash-preview';
  
  const systemInstruction = `Analyze the user's question and suggest the top 3-5 most relevant strategic answer styles from the following list:
  Psychological Projection, Executive Summary, Analogy Based, Data-Driven Insights, Concise Answer, In-Depth Response, Answer in Points, Define Technical Terms, Sales Points, Key Statistics, Case Study Summary, Competitive Comparison, Anticipated Customer Questions, Information Gap, Pricing Overview, ROI Forecast, SWOT Analysis, Strategic Roadmap, Risk Assessment, Implementation Timeline, Technical Deep-Dive, Value Proposition, Financial Justification, Stakeholder Alignment, Competitive Wedge, Success Story Summary, Buying Fear Mitigation, Security & Compliance, Decision Matrix, Reasoning Chain.
  ...
  try {
    ...
    console.error("Option suggestion failed:", error);
    return ["Executive Summary", "Strategic Roadmap", "Value Proposition"];
  }
}
